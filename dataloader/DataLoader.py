...
con = mysql.connector.connect(
            ...
            password=password,
            host='127.0.0.1',
            database='dataLoader', auth_plugin='mysql_native_password')
curseur= con.cursor()
...
for MOIS in range(len(lesMois)):
    ...
    if(os.path.getsize(file)!=0):
                                ...
                                for n in enumerate(json_obj[elt][k[1]][j[1]][o[1]]):
                                    ...
                                    if(len(Regin)!=0):
                                        for i in Regin:
                                            if("DAKAR_"+j[1]==i[0]):
                                                #print("Dans la base")
                                                request2 = "SELECT Nbre_Cas from "+"`"+elt+"`"+" WHERE Localites="+"'"+i[0]+"'"+""
                                                curseur.execute(request2)
                                                Anc_Nbre_Cas=curseur.fetchone()
                                                New_Nbre_Cas=Anc_Nbre_Cas[0]+int(n[1])
                                                request3 = "UPDATE "+"`"+elt+"`"+" SET Nbre_Cas="+str(New_Nbre_Cas)+" WHERE Localites="+"'"+i[0]+"'"+""
                                                #print(request3)
                                    ...
                                    if(len(Regin)!=0):
                                        for i in Regin:
                                            if(k[1]==i[0]):
                                                ...
                                                curseur.execute(request2)
                                                Anc_Nbre_Cas=curseur.fetchone()
                                                New_Nbre_Cas=Anc_Nbre_Cas[0]+int(o[1])
                                                request3 = "UPDATE "+"`"+elt+"`"+" SET Nbre_Cas="+str(New_Nbre_Cas)+" WHERE Localites="+"'"+i[0]+"'"+""
                                                #print(request3)
                                                curseur.execute(request3)
                                            else:
                                        ...
                                        request2 = "INSERT IGNORE INTO "+"`"+elt+"`"+"(Localites,Nbre_Cas) values("+"'DAKAR_"+str(k[1])+"'"+","+"'"+str(o[1])+"'"+")"
                                        curseur.execute(request2)
                else:
                    Liste=[]
                    if (k[1]=="id_com"):
        ...
        var.set("Voulez vous valider l'importation du mois suivant:\n"+lesMois[MOIS].upper()+"" )
